...
result$clust$models <- log_mlik_all(
    result$clust$membership, stgaus, names(st_dimensions(stgaus)),
    correction = FALSE, detailed = TRUE,
    formula = y ~ f(idt, model = "rw1", hyper = list(prec = list(prior = "normal", param = c(-2, 1))))
)
result
```
...
result2$clust$models <- log_mlik_all(
    result2$clust$membership, stgaus, names(st_dimensions(stgaus)),
    correction = FALSE, detailed = TRUE,
    formula = y ~ f(idt, model = "rw1", hyper = list(prec = list(prior = "normal", param = c(-2, 1))))
)
result2
