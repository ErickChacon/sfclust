...
#     niter = 1000, path_save = file.path("inst", "vigdata", "binomial-mcmc.rds")
#   )
# )
# #     user   system  elapsed
# # 5207.915  712.998  831.280
```

```{r, eval = FALSE}
set.seed(7)
result <- sfclust(stbinom, formula = cases ~ poly(time, 2) + f(id),
  family = "binomial", Ntrials = population, niter = 1000)
names(result)
```
...
Finally, the log marginal likelihood for the last iteration (1000) is reported as
-61,286.28.
...
the 1000th (final) iteration. The middle panel shows the mean linear predictor curves for
...
confirms that it corresponds to the 1000th sample out of a total of 1000. It also displays
...
this case, Cluster 1 has 26 members, Cluster 2 has 12 members, and so on. Finally, it
