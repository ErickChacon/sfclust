...
`c("geometry", "time")`, you must specify the `stnames` argument when using the package.
...
3. An unstructured random effect across states (`id`)

We begin with one cluster per state (49 clusters total). Be sure to set `stnames =
c("space", "time")`.
...
The `summary` indicates that Out of 11 clusters, 7 include more than one state. The
largest cluster contains 10 states, followed by clusters with 9, and so on.
...
* Mean relative risk increases gradually across the year.
* The marginal likelihood improves within the first 100 iterations (after thinning) and stabilizes afterward.
...
We use the cluster assignments to analyze and visualize the empirical relative risk per cluster.
