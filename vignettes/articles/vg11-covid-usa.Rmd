...
the default `"geometry"`, you must specify `sp_dims = "space"` when using `sfclust()`.
...
2. An autoregressive effect over `idf_time`
...
dimension name: `sp_dims = "space"`.

```{r, eval = FALSE, echo = FALSE}
# formula <- cases ~ 1 + poly(time, 3) + f(idf_time, model = "ar1") + f(id)
# geodata <- genclust(usacovid, nclust = 49)
#
# set.seed(123)
# result <- sfclust(usacovid, graphdata = geodata, sp_dims = "space",
