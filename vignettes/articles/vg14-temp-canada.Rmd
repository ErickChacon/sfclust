...
`sfclust` assumes that the `geometry` type of the provided `stars` object if `POLYGON`,
from which it create a `igraph` using the adjacency matrix. However, the processed
...
```{r, fig.height = 3}
plot(result, sort = TRUE, legend = TRUE, geom_before = geom_sf(data = canada), clusters = 1:8)
```
...
```{r, echo = FALSE}
path_figures <- here::here(file.path("tools", "figures"))
ggsave(file.path(path_figures, "canweather-map.pdf"), width = 6, height = 6, device = cairo_pdf)
```
...
```{r, echo = FALSE}
ggsave(file.path(path_figures, "canweather-risk-per-cluster.pdf"), width = 10, height = 7, device = cairo_pdf)
```
