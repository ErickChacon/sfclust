...
```{r}
canada <- rnaturalearth::ne_states("Canada")
stations$cluster <- factor(sfclust:::sort_membership(result$clust$membership))

ggplot() +
  geom_sf(data = st_transform(canada, st_crs(stations))) +
  geom_sf(aes(color = cluster), data = stations, size = 2) +
  labs(color = NULL) +
  theme_bw()
```
...
```{r, echo = FALSE}
ggsave(file.path(path_figures, "canweather-risk-per-cluster-1.pdf"), width = 10, height = 5.5, device = cairo_pdf)
```
