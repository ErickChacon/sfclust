...
<<<<<<< Updated upstream
=======
Then we will create a stars object.

```{r}
time_seq <- seq.Date(from = as.Date("2022-01-01"), by = "day", length.out = dim(Y)[1])
coords <- st_centroid(geodata)
nt <- dim(Y)[1]
ns <- dim(Y)[2]
# Convert the Y matrix into an array with dimensions: time x space
Y_array <- array(Y, dim = c(nt, ns))

# Create the stars object
stars_obj <- st_as_stars(
  Y = Y_array,
  dimensions = st_dimensions(
    time = time_seq,
    space = coords$geometry
  )
)
...
In this section, we show how to use Bsfclust algorithm to find the spatial clusters. First we will create a stars object.
...
X1_expanded <- matrix(rep(X1, each = 100), nrow = 100, ncol = 100)
X2_expanded <- matrix(rep(X2, each = 100), nrow = 100, ncol = 100)


# Combine X1, X2, and Y into a single stars object
data <- st_as_stars(Y = Y, X1 = X1_expanded, X2 = X2_expanded)
st_dimensions(data) <- st_dimensions(space = coords$geometry, time = time_seq)
map <- geodata
```
