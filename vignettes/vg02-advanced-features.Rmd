...
#   formula = y ~ f(idf_time, model = "rw1", hyper = list(prec = list(prior = "normal", param = c(-2, 1)))),
...
```{r, eval = TRUE}
result <- update(result0, niter = 20)
result
```
...
```{r}
result_other <- sfclust:::update_within(result, sample = nrow(result$samples$membership)/2)
result_other
```
