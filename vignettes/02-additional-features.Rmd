...
The simulated dataset used in this vignette, `stgauss`, is included in our package. It is
...
stweekly <- aggregate(stgauss, by = "week", FUN = mean)
ggplot() +
...
stgauss |>
    st_set_dimensions("geometry", values = 1:nrow(stgauss)) |>
    as_tibble() |>
    ggplot() +
...
```{r}
set.seed(123)
initial_cluster <- genclust(st_geometry(stgauss), nclust = 20)
...
```{r}
st_sf(st_geometry(stgauss), cluster = factor(initial_cluster$membership)) |>
  ggplot() +
...
# result <- sfclust(stgauss, graphdata = initial_cluster, niter = 50,
#     formula = y ~ f(idt, model = "rw1", hyper = list(prec = list(prior = "normal", param = c(-2, 1)))),
#   path_save = file.path("inst", "vigdata", "normal-mcmc1.rds"))
# system.time(
#   update(result, niter = 2000, path_save = file.path("inst", "vigdata", "normal-mcmc2.rds"))
# )
#      user    system   elapsed
# 15953.007 11858.150  2781.883
```
...
```{r, echo = FALSE}
result <- readRDS(system.file("vigdata", "normal-mcmc1.rds", package = "sfclust"))
result
```
...
```{r, echo = FALSE}
result2 <- readRDS(system.file("vigdata", "normal-mcmc2.rds", package = "sfclust"))
result2
```
...
```{r}
stgauss$cluster <- fitted(result2, sort = TRUE)$cluster
st_sf(st_geometry(stgauss), cluster = factor(stgauss$cluster)) |>
  ggplot() +
    geom_sf(aes(fill = cluster)) +
    theme_bw()
```
